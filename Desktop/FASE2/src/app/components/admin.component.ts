...
export class AdminComponent implements OnInit {
  ...
  onDeleteEvent(eventId: string): void {
    const doRefund = confirm('¿Deseas devolver el dinero a los compradores de este evento? (Aceptar = Sí)');
    const currentUser = this.authService.currentUser();
    this.eventService.cancelEvent(eventId, doRefund, currentUser?.username);
    // opcional: mostrar confirmación local al admin
    alert('Evento eliminado. ' + (doRefund ? 'Se han generado devoluciones.' : 'No se realizaron devoluciones.'));
  }
